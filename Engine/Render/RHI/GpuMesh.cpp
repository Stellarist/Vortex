    ...
    context(&context), submesh(&submesh)
{
	...
	object_descriptor = std::make_unique<DescriptorSet>(context, pool.getSet(set_index).get());
	object_descriptor->update(0, vk::DescriptorType::eUniformBuffer, object_uniform.get());
}
...
const DescriptorSet* GpuMesh::getDescriptor() const
{
	return object_descriptor.get();
}

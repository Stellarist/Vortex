...
uint32 getMipExtent(uint32 extent, uint32 mip_level)
{
...
RHITextureSlice normalizeTextureSlice(const RHITextureDesc& desc, const RHITextureSlice& slice)
{
	...
	assert(static_cast<uint32>(slice.x) < mip_width &&
	...
	assert(result.width > 0 && result.height > 0 && result.depth > 0 && static_cast<uint32>(slice.x + result.width) <= mip_width &&
	    static_cast<uint32>(slice.y + result.height) <= mip_height && static_cast<uint32>(slice.z + result.depth) <= mip_depth &&

...
{
	if (!pass.desc.render_targets || (pass.desc.render_targets->colors.empty() && !pass.desc.render_targets->depth))
		return {};

	auto* first_texture = pass.desc.render_targets->colors.empty() ?
	    pass.desc.render_targets->depth->texture :
	    pass.desc.render_targets->colors.front().texture;
	...
	auto requireTexture = [&](RDGTextureRef texture) -> RDGTexture& {
		CHECK(texture->texture, "RDG raster pass '{}' uses an unallocated attachment '{}'", pass.desc.name, texture->name);
		return *texture;
	};
	...
	if (pass.desc.render_targets->depth) {
		const auto& attachment = *pass.desc.render_targets->depth;
...
{
	for (auto* view : pass.desc.access.texture_views) {
		if (view->view)
			continue;
	...
	for (auto* view : pass.desc.access.buffer_views) {
		if (view->view)
			continue;
...
void RDGBuilder::calculateCulling()
{
	...
	for (uint32 pass_index = 0; pass_index < graph.passes.size(); ++pass_index) {
		...
		if (hasAnyFlags(pass.desc.flags, RDGPassFlags::NeverCull))
...
void RDGBuilder::execute(RHIDevice& device, RHICommandList& command)
{
	...
	for (auto handle : execution_order) {
		...
		auto framebuffer = pass.desc.type == RDGPassType::Raster ?
		    createFramebuffer(device, pass) :
		    RHIRef<RHIFramebuffer>{};
		if (framebuffer)
			framebuffer->setName(pass.desc.name);

		RDGPassContext context(graph, device, command, framebuffer.get(), pass.desc);
		createPassViews(device, pass);

		command.beginDebugLabel(pass.desc.name);
		if (framebuffer)
...
RDGPassHandle RDGBuilder::addPass(RDGPassDesc desc,
    std::function<void(RDGPassContext&)> execute)
{
	CHECK(!executed, "Cannot add a pass after the RDG has executed");
	CHECK(Argument, execute, "Cannot add an RDG pass without an execute function");
	validateRDGName(desc.name, "pass");

	const auto pass_count = graph.passes.size();
	graph.passes.push_back({std::move(desc), {}, std::move(execute)});

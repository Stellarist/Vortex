...
void validateRDGTextureAccess(const RDGPassNode& pass, const RDGTexture& texture, const RDGResourceAccess& access)
{
	...
	CHECK(valid,
	    ...
	    pass.desc.name, static_cast<uint16>(access.state), texture.name);
}
...
void validateRDGBufferAccess(const RDGPassNode& pass, const RDGBuffer& buffer, const RDGResourceAccess& access)
{
	...
	CHECK(valid, "RDG pass '{}' state {} is incompatible with buffer '{}' usage",
	    pass.desc.name, static_cast<uint16>(access.state), buffer.name);
}
...
void validateRDGResourceAccess(const RDGGraph& graph, const RDGPassNode& pass, const RDGResourceAccess& access)
{
	CHECK(access.resource && access.resource->owner == &graph,
	    "RDG pass '{}' uses a resource owned by another graph", pass.desc.name);

	CHECK(access.state != Unknown, "RDG pass '{}' declares an unknown state", pass.desc.name);
	...
	if (access.resource->type == RDGResourceType::Texture) {
		CHECK(
		    ...
		    "RDG pass '{}' uses an invalid texture reference", pass.desc.name);
	...
	CHECK(
	    ...
	    "RDG pass '{}' uses an invalid buffer reference", pass.desc.name);
...
{
	CHECK(pass.desc.render_targets, "Raster RDG pass requires render target declarations");
	...
	auto include = [&](const RDGTexture& texture) {
		...
		CHECK(
		    ...
		    "RDG raster pass '{}' has attachments with different extents", pass.desc.name);

		CHECK(texture.desc.sample_count == reference->desc.sample_count,
		    "RDG raster pass '{}' has attachments with different sample counts", pass.desc.name);
	};

	for (const auto& attachment : pass.desc.render_targets->colors) {
		...
		CHECK(!isRHIDepthFormat(texture.desc.format),
		    ...
		    pass.desc.name, texture.name);

		include(texture);
	}

	if (pass.desc.render_targets->depth) {
		const auto& texture = *pass.desc.render_targets->depth->texture;
		CHECK(isRHIDepthFormat(texture.desc.format),
...
void validateRDGCopyPass(const RDGPassNode& pass)
{
	...
	for (const auto& access : pass.desc.access.accesses) {
		if (access.resource->type != RDGResourceType::Texture)
			continue;
	...
	CHECK(compatible,
	    ...
	    pass.desc.name, source->name, destination->name);
}
...
void validateRDGPass(const RDGGraph& graph, RDGPassHandle handle)
{
	...
	CHECK(pass.execute, "RDG pass {} has no execute function", handle.index);
	CHECK(!pass.desc.name.empty(), "RDG pass {} has no name", handle.index);


	CHECK(pass.desc.type == RDGPassType::Raster || !pass.desc.render_targets.has_value(),
	    "Non-raster RDG pass '{}' declares raster attachments", pass.desc.name);

	for (const auto& access : pass.desc.access.accesses)
		validateRDGResourceAccess(graph, pass, access);

	if (pass.desc.type == RDGPassType::Raster)
		validateRDGRasterPass(pass);

	if (pass.desc.type == RDGPassType::Copy)

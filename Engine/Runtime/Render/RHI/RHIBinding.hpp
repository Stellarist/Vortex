...
struct RHIBindingLayoutItem {
	...
	RHIBindingLayoutItem& setSlot(uint32_t new_slot) noexcept
	{
		slot = new_slot;
		return *this;
	}

	RHIBindingLayoutItem& setCount(uint32_t new_count) noexcept
	{
		count = new_count;
		return *this;
	}
	...
	RHIBindingLayoutItem& setPushConstants(uint32_t new_size, uint32_t new_slot = 0) noexcept
	{
...
struct RHIBindingSetItem {
	...
	RHIBindingSetItem& setSlot(uint32_t new_slot) noexcept
	{
		slot = new_slot;
		return *this;
	}
	...
	RHIBindingSetItem& setTextureView(
	    uint32_t new_slot, RHITextureView* new_texture_view, RHIBindingType new_type = RHIBindingType::TextureSRV) noexcept
	{
		slot = new_slot;
		type = new_type;
	...
	RHIBindingSetItem& setSampler(uint32_t new_slot, RHISampler* new_sampler) noexcept
	{
		slot = new_slot;
	...
	RHIBindingSetItem& setBufferView(
	    uint32_t new_slot, RHIBufferView* new_buffer_view, RHIBindingType new_type = RHIBindingType::ConstantBuffer) noexcept
	{
		slot = new_slot;
		type = new_type;
	...
	RHIBindingSetItem& setPushConstants(uint32_t new_slot = 0) noexcept
	{
		slot = new_slot;
		type = RHIBindingType::PushConstants;
		resource = nullptr;

...
{
	assert(!bytecode.empty() && (bytecode.size() % sizeof(uint32_t)) == 0 && "Shader bytecode must contain non-empty, 4-byte aligned SPIR-V data.");
...
void VulkanDevice::bindBufferMemory(RHIBuffer* buffer, uint64_t offset) const noexcept
{
	auto* vk_buffer = static_cast<VulkanBuffer*>(buffer);
	if (!vk_buffer)
		return;
...
void VulkanDevice::bindTextureMemory(RHITexture* texture, uint64_t offset) const noexcept
{
